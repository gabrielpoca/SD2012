...
public class Peer extends Thread {
	...
	public Peer(int port, Mutex mutex) {
		this.port = port;
		this.mutex = mutex;
	}
			...
			while (run) {
				...
				while (keys.hasNext() && run) {
					...
					if (!key.isValid())
						continue;
					if (key.isAcceptable()) {
						ServerSocketChannel key_server_channel = (ServerSocketChannel) key
								.channel();
						SocketChannel channel = key_server_channel.accept();
						channel.configureBlocking(false);
						SelectionKey nkey = channel.register(key.selector(),
	...
	public static void main(String[] args) {
		...
		t_mutex.start();
		Peer peer = new Peer(Integer.valueOf(args[3]), mutex);
