...
public class Mutex extends Thread {
    ...
    /**
     * Mutex uses lock to find if there is a someone waiting for the mutex.
     */
...
//			server_channel.register(selector, SelectionKey.OP_READ);
    ...
    public void unlock() {
	if(lock.handlerReturnMutex())
	    sendMutex();
    }

    /**
     * Used to send to mutex to the next peer in the ring. It accomplishes it by
     * establishing a connection to the write_port.
     */
