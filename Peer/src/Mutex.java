...
public class Mutex extends Thread {
	...
	public void run() {
		try {
			...
			serverChannel.socket().bind(
					new InetSocketAddress("localhost", READ_PORT));
			...
			while (run) {
				...
				while (keys.hasNext() && run) {
					...
					if (key.isAcceptable()) {
						ServerSocketChannel server_channel = (ServerSocketChannel) key
								.channel();
	...
	private void release() {
		try {
			...
			DataOutputStream output_stream = new DataOutputStream(
